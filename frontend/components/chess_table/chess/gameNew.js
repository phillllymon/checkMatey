...
export class Game {
    ...
    initializeGameMap() {
        let gameMap = [];
        for (let i = 0; i < 8; i++) {   //populate gameMap with empty all false dests and origins
            let row = [];
            for (let j = 0; j < 8; j++) {
                row.push({origins: this.makeFalseGrid(), dests: this.makeFalseGrid()});
            }
            gameMap.push(row);
        }
        for (let i = 0; i < 8; i++) {   //now get moves for all the pieces
            for (let j = 0; j < 8; j++) {
                let mark = this.grid[i][j];
                if (mark !== '-') {
                    let pieceType = getPieceType(mark);
                    let color = getPieceColor(mark);
                    let origin = [i, j];
                    let moves = getPieceDumbMoves(origin, pieceType, color, this.grid);
                    let dests = gameMap[i][j].dests;
                    moves.forEach((move) => {
